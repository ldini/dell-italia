...
export class ZonasComponent implements OnInit {
  ...
  onEnableClick(id: string) {
    ...
    this.confirmDialogService.showConfirm("Desea dar de alta la zona?", function () {  
      apiService.DoPOST<ApiResult<any>>("zonas/enable?encryptedId=" + encodeURIComponent(id), {}, /*headers*/ null,
                                            (response) => {
                                              if (!response.success) {
                                                confirmDialogService.showError(response.message);
                                              }
                                              else {
                                                notifier.notify('success', 'Zona dada de alta con éxito.');
  ...
  onDisableClick(id: string) {
    ...
    this.confirmDialogService.showConfirm("Desea dar de baja la zona?", function () {  
      apiService.DoDELETE<ApiResult<any>>("zonas/disable?encryptedId=" + encodeURIComponent(id), /*headers*/ null,
                                            (response) => {
                                              if (!response.success) {
                                                confirmDialogService.showError(response.message);
                                              }
                                              else {
                                                notifier.notify('success', 'Zona dada de baja con éxito.');
                                                dataTableInstance.then((dtInstance: DataTables.Api) => {
                                                  dtInstance.ajax.reload()
                                                });
                                              }
  ...
  ngOnInit(): void {
    ...
    this.dtIndex = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.apiService.DoPOST<ApiResult<DataTableDTO<ZonaDTO>>>("zonas/list?status=" + this.filterStatusValue, dataTablesParameters, /*headers*/ null,
                      (response) => {
                        if (!response.success) {
