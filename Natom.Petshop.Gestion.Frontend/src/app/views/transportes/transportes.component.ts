...
export class TransportesComponent implements OnInit {
  ...
  onEditClick(id: string) {
    this.routerService.navigate(['/transportes/edit/' + encodeURIComponent(id)]);
  }
  ...
  onEnableClick(id: string) {
    ...
    this.confirmDialogService.showConfirm("Desea dar de alta al transporte?", function () {  
      apiService.DoPOST<ApiResult<any>>("transportes/enable?encryptedId=" + encodeURIComponent(id), {}, /*headers*/ null,
                                            (response) => {
                                              if (!response.success) {
                                                confirmDialogService.showError(response.message);
                                              }
                                              else {
                                                notifier.notify('success', 'Transporte dado de alta con éxito.');
  ...
  onDisableClick(id: string) {
    ...
    this.confirmDialogService.showConfirm("Desea dar de baja al transporte?", function () {  
      apiService.DoDELETE<ApiResult<any>>("transportes/disable?encryptedId=" + encodeURIComponent(id), /*headers*/ null,
                                            (response) => {
                                              if (!response.success) {
                                                confirmDialogService.showError(response.message);
                                              }
                                              else {
                                                notifier.notify('success', 'Transporte dado de baja con éxito.');
                                                dataTableInstance.then((dtInstance: DataTables.Api) => {
                                                  dtInstance.ajax.reload()
                                                });
                                              }
  ...
  ngOnInit(): void {
    ...
    this.dtIndex = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.apiService.DoPOST<ApiResult<DataTableDTO<TransporteDTO>>>("transportes/list?status=" + this.filterStatusValue, dataTablesParameters, /*headers*/ null,
                      (response) => {
                        if (!response.success) {
                          ...
                          this.Transportes = response.data.records;
